...
<div class="min-h-screen py-12">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <!-- Form -->
        <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data" class="bg-black/40 backdrop-blur-md rounded-xl border border-cyan-500/30 p-8">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="col-span-2">
                    ...
                    <textarea name="description" id="description" rows="4" required
                        class="w-full px-4 py-3 bg-black/50 border border-cyan-500/30 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500">{{ old('description', $product->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                @if($product->images && count($product->images) > 0)
                    <div class="col-span-2">
                        <label class="block text-sm font-medium text-cyan-400 mb-2">Current Images</label>
                        ...
                        <p class="mt-2 text-sm text-gray-400">Uploading new images will replace all current images</p>
                    </div>
                @endif

                <!-- New Images -->
                <div class="col-span-2">
                    <label for="images" class="block text-sm font-medium text-cyan-400 mb-2">Upload New Images (Optional)</label>
                    ...
                    <p class="mt-1 text-sm text-gray-400">Leave empty to keep current images. Accepted formats: JPEG, PNG, JPG, WebP (Max: 2MB each)</p>
                    @error('images.*')
                        <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="col-span-2" id="image-preview-container" style="display: none;">
                    <label class="block text-sm font-medium text-cyan-400 mb-2">New Images Preview</label>
