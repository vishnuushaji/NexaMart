                    ...
                    <tbody class="divide-y divide-gray-700">
                        @forelse($categories as $category)
                            <tr class="hover:bg-white/5 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="text-sm text-cyan-400">{{ $category->products_count ?? 0 }}</span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex space-x-2">
                                        ...
                                        <form action="{{ route('admin.categories.destroy', $category) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure? This category has {{ $category->products_count ?? 0 }} products.');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="text-red-400 hover:text-red-300 {{ ($category->products_count ?? 0) > 0 ? 'opacity-50 cursor-not-allowed' : '' }}" {{ ($category->products_count ?? 0) > 0 ? 'disabled' : '' }}>
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
