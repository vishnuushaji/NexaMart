...
<div class="min-h-screen py-12">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <!-- Form -->
        <form action="{{ route('admin.categories.update', $category) }}" method="POST" class="bg-black/40 backdrop-blur-md rounded-xl border border-cyan-500/30 p-8">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3"
                        class="w-full px-4 py-3 bg-black/50 border border-cyan-500/30 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500">{{ old('description', $category->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="bg-black/30 rounded-lg p-4 border border-cyan-500/20">
                    <h3 class="text-sm font-medium text-cyan-400 mb-2">Category Information</h3>
                    <div class="space-y-1 text-sm text-gray-400">
                        <p>Slug: <span class="text-white">{{ $category->slug }}</span></p>
                        <p>Products in this category: <span class="text-white">{{ $category->products()->count() }}</span></p>
                        <p>Created: <span class="text-white">{{ $category->created_at->format('M d, Y') }}</span></p>
                        <p>Last updated: <span class="text-white">{{ $category->updated_at->format('M d, Y') }}</span></p>
                    </div>
                </div>
            </div>
...
@push('scripts')
<script>
    // Auto-generate slug from category name (optional)
